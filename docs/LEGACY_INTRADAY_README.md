...
**Milestone 19 — Master Risk Engine (Swing Trading Hedge Fund)** (complete)

Pivoted the infrastructure to support a fully-automated, capital-constrained **Swing Trading Hedge Fund**. The system now runs daily via GitHub Actions, tracks 3 separate multi-day swing strategies, and unifies them under a strict ₹2,00,000 total capital limit.

**What was added:**

| File | Description |
|---|---|
| `scripts/run_master_trader.py` | The unified risk engine. Aggregates open positions, calculates free cash, and dynamically sizes trades into minimum ₹50,000 chunks. |
| `scripts/backtest_master_engine.py` | A 5-year historical simulation engine that mathematically proves the portfolio's constrained capital performance (+110% return over 5 years). |
| `scripts/run_ma_pullback_trader.py` | Daily tracker for the Moving Average Pullback strategy (ranked by lowest RSI). |
| `scripts/run_bb_squeeze_trader.py` | Daily tracker for the Bollinger Band Squeeze Breakout strategy (ranked by tightest bandwidth). |
| `scripts/run_paper_trader.py` | Daily tracker for the Long-Only Black Swan pairs strategy (ranked by absolute Z-Score). |
| `.github/workflows/daily_trader.yml` | Fully automated cloud orchestrator that triggers the Master Risk Engine at 3:15 PM daily and texts the operator via Telegram. |

**Key features:**

- **Dynamic Fractional Sizing:** Protects against over-allocation. Never exceeds the ₹2 Lakh hard cap, no matter how many signals fire.
- **Mathematical Signal Ranking:** When capital is scarce, the engine ranks competing signals by mathematical edge (e.g., prioritizing the deepest RSI panics over shallower pullbacks) to select the optimal trades.
- **Unified Telegram Digest:** Instead of spamming multiple logs, it generates a single, elegant daily text message showing: Free Cash, Locked Cash, Approved Entries with dynamic quantities, Rejected Entries, Exits, and Unrealized PnL.
